...
<script>
import { EDIT_REASONEXEMPTION } from "@/store/reasonExemptions/reasonExemption.constants";
...
export default {
  ...
  computed: {
    ...mapGetters("reasonExemption", ["getReasonExemptionById", "getMessage"])
  },
  methods: {
    update() {
      this.$store
        .dispatch(
          `reasonExemption/${EDIT_REASONEXEMPTION}`,
          this.$data.reasonExemption
        )
        .then(
          () => {
            this.$alert(
              this.getMessage,
              "Razão de isenção atualizada!",
              "success"
            );
  ...
  created() {
    this.reasonExemption = this.getReasonExemptionById(
      this.$route.params.reasonExemptionId
    );
  }
};
</script>
