                  ...
                  <b-row>
                    <b-col>
                      <div class="input-group">
                        ...
                        <input
                          v-model="withholdingTaxPercentage"
                          class="form-control form-control-sm"
...
<script>
import { ADD_CUSTOMER } from "@/store/customers/customer.constants";
...
export default {
  ...
  computed: {
    ...mapGetters("customer", ["getMessage"]),
    ...mapGetters("naturality", ["getNaturalities"])
  },
  methods: {
    add() {
      this.$store.dispatch(`customer/${ADD_CUSTOMER}`, this.$data).then(
        () => {
          this.$alert(this.getMessage, "Cliente adicionado!", "success");
