...
<script>
import { ADD_SUPPLIER } from "@/store/suppliers/supplier.constants";
...
export default {
  ...
  computed: {
    ...mapGetters("supplier", ["getMessage"]),
    ...mapGetters("naturality", ["getNaturalities"])
  },
  methods: {
    add() {
      this.$store.dispatch(`supplier/${ADD_SUPPLIER}`, this.$data).then(
        () => {
          this.$alert(this.getMessage, "Fornecedor adicionado!", "success");
