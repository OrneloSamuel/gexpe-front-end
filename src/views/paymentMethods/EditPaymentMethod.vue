...
<script>
import { EDIT_PAYMENTMETHOD } from "@/store/paymentMethods/paymentMethod.constants";
...
export default {
  ...
  computed: {
    ...mapGetters("paymentMethod", ["getPaymentMethodById", "getMessage"])
  },
  methods: {
    update() {
      this.$store
  ...
  created() {
    this.paymentMethod = this.getPaymentMethodById(
      this.$route.params.paymentMethodId
    );
  }
};
</script>
