...
<script>
import { EDIT_PRODUCTCATEGORY } from "@/store/productCategories/productCategory.constants";
...
export default {
  ...
  computed: {
    ...mapGetters("productCategory", ["getProductCategoryById", "getMessage"]),
  },
  methods: {
    update() {
      this.$store
        .dispatch(
          `productCategory/${EDIT_PRODUCTCATEGORY}`,
  ...
  created() {
    this.productCategory = this.getProductCategoryById(
      this.$route.params.productCategoryId
    );
  }
};
</script>
